<script setup>
    ...
    const get_mouse_pos = (e) => {
        return {
            x: e.clientX + document.documentElement.scrollLeft - map_canvas.value.offsetLeft,
            y: e.clientY + document.documentElement.scrollTop - map_canvas.value.offsetTop
        }
    }
    ...
    const get_map_coords = (e) => {
        ...
        return {
            x: (Math.floor(mouse_pos.x / TILE_SIZE) + (coords.x + 1) + (((mouse_pos.x % TILE_SIZE) - (TILE_SIZE - map_canvas.value.scrollLeft)) / TILE_SIZE)) / Math.pow(2, coords.z),
            y: (Math.floor(mouse_pos.y / TILE_SIZE) + (coords.y + 1) + (((mouse_pos.y % TILE_SIZE) - (TILE_SIZE - map_canvas.value.scrollTop)) / TILE_SIZE)) / Math.pow(2, coords.z)
        }
    }
