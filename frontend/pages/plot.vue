                ...
                <div v-show="update_display == false">
                    ...
                    <ul class="list-group mb-3">
                        <div v-for="task in plot.tasks">
                            <li class="list-group-item d-flex justify-content-between align-items-center" v-if="task.checked">
                                {{ task.name }}
    ...
    onMounted(() => {
        ...
        Promise.all(get_promises).then(() => {
            ...
            send_api("GET", "tasks").then((response) => {
                plot.value.tasks = JSON.parse(response.response)
                for (let i = 0; i < plot.value.tasks.length; i++) {
                    plot.value.tasks[i].checked = false
                    plot.value.tasks[i].checked_db = false
                }
                retrieve_plot_season_plot_tasks(plot.value.id, settings_store.current_season).then((plot_tasks_api) => {
                    for (let plot_task of plot_tasks_api) {
                        // Find the corresponding task in plot.value.tasks
                        for (let i = 0; i < plot.value.tasks.length; i++) {
